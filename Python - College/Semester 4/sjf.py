def sjf(debug) -> None:
    ...
    while temp_list:
        ...
        least_burst_time = min(temp_list, key=lambda process: process[2])
        current_time += least_burst_time[2]
        Gantt_Chart.append((least_burst_time[0], current_time))
        temp_list.remove(least_burst_time)

        # Compute the necessary values -> Completion time, Turn Around time, Waiting time
        CT = current_time
        TAT = CT - least_burst_time[1]
        WT = TAT - least_burst_time[2]
        index = Processes.index(least_burst_time)
        Processes[index] = (
            *least_burst_time,
            CT,
            TAT,
